...
export function calcPagesV1(_title: string, paragraphs: string[], options: ReaderConfig) {
...
function getCharWidth({ font, size, char }: { font: string, size: number, char: string }): number {
    if (!cache?.[font]?.[size]?.[char]) {
        ...
        const _width = ctx.measureText(char).width;

        cache[font] ||= {};
        cache[font][size] ||= {};
        cache[font][size][char] = _width;
    }
    return cache[font][size][char];
}

export function clearCacheV1() {
    cache = {};
}